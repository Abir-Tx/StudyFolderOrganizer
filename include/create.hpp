...
// Function
bool IsNumber(std::string s) /* Function for checking string values if it has
...
void createFolder(std::string driveLetter) {
    ...
    do {
        ...
        do {
            ...
            // Handling the error where if users input string values the program
            // will not crash
        ...
        case 1: {
            ...
            if (choice == 1) {
                ...
                std::cout << "Bad input. Going with automatic creation"
                          << std::endl;
                isManualCreationChoice_Lab = true;
            }
            ...
            if (isManualCreationChoice_Lab == true) {
                ...
                selectedFoldersName_Lab.push_back("Lab_Tasks");
            }
            ...
            system("cls");
            // Lab Creation
            std::string labPath = parentDir + year + "st Year\\" + semester +
                                  "th Semester\\" + "Lab";
            ...
            // Term creation
            for (int i = 0; i < labSubjectsNumber; i++) {
                std::string labMidTermPath =
                    parentDir + year + "st Year\\" + semester +
                    "th Semester\\" + "Lab\\" + labSubjects[i] + "\\Mid-Term";
                _mkdir(labMidTermPath.c_str());
            }
            for (int i = 0; i < labSubjectsNumber; i++) {
                std::string labFinalTermPath =
                    parentDir + year + "st Year\\" + semester +
                    "th Semester\\" + "Lab\\" + labSubjects[i] + "\\Final";
            ...
            for (int i = 0; i < labSubjects.size(); i++) {
                ...
                while (k != selectedFoldersNumber_Lab) {
                    for (int j = 0; j < selectedFoldersNumber_Lab; j++) {
                        std::string midTermPath =
                            parentDir + year + "st Year\\" + semester +
                            "th Semester\\" + "Lab\\" + labSubjects[i] +
                            "\\Mid-Term\\" + selectedFoldersName_Lab[j];
                        _mkdir(midTermPath.c_str());

                        std::string finalTermPath =
                            parentDir + year + "st Year\\" + semester +
                            "th Semester\\" + "Lab\\" + labSubjects[i] +
                            "\\Final\\" + selectedFoldersName_Lab[j];
        ...
        case 2: {
            ...
            for (int i = 0; i < subjects.size(); i++) {
                std::string midTermPath =
                    parentDir + year + "st Year\\" + semester +
                    "th Semester\\" + "Theory\\" + subjects[i] + "\\Mid-Term";
                _mkdir(midTermPath.c_str());
            }
            for (int i = 0; i < subjects.size(); i++) {
                std::string finalTermPath =
                    parentDir + year + "st Year\\" + semester +
                    "th Semester\\" + "Theory\\" + subjects[i] + "\\Final";
                _mkdir(finalTermPath.c_str());
            }
                ...
                while (k != selectedFoldersNumber) {
                    for (int j = 0; j < selectedFoldersNumber; j++) {
                        std::string midTermPath =
                            ...
                            "\\Mid-Term\\" + selectedFoldersName[j];
                        _mkdir(midTermPath.c_str());

                        std::string finalTermPath =
                            parentDir + year + "st Year\\" + semester +
                            "th Semester\\" + "Theory\\" + subjects[i] +
                            "\\Final\\" + selectedFoldersName[j];
                        _mkdir(finalTermPath.c_str());
                    }
                    k++;
                }
